...
public partial class HeadBoneClickToggle : Control
{
	...
	{
		_spineNode = FindSpineSprite();
		if (_spineNode == null)
		{
			GD.PrintErr("选人场景：未找到 SpineSprite 或 Visuals，本地插槽控制将不可用");
		}
		else
		{
		...
		{
			HeadVisibilityBus.SetVisibility(pressed);
		...
		if (_skeleton != null)
		{
			_isHidden = HeadVisibilityBus.IsHidden;
			ApplyLocalVisibility();
		}
	}

	private Node FindSpineSprite()
	{
		var node = GetNode("../SpineSprite");
		if (node != null) return node;

		node = GetNode("../Visuals");
		if (node != null) return node;
		
		return GetTree().CurrentScene?.FindChild("SpineSprite", true, false);
	}
	...
	{
		if (_skeleton == null || _targetSlots.Count == 0) return;
